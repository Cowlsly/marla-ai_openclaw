    ...
    async ({ attachment, gesture, expectedTurns }) => {
      await suite.withPage(
        ...
        async ({ page }) => {
          ...
          await pane.getByText(terminalText, { exact: true }).waitFor();
          ...
          try {
            ...
            const rows = pane.locator(".chat-group.user .chat-bubble", {
              hasText: attachment ? fileName : followUpText,
            });
            const observed = await rows.evaluateAll((bubbles) =>
              bubbles.map((bubble) => {
            ...
            expect(observed.every((row) => row.visible && row.id)).toBe(true);
            expect(new Set(observed.map((row) => row.id)).size).toBe(observed.length);
            expect(observed.map((row) => row.id)).toContain(`msg:send:${followUpRunId}:0`);
            expect(observed).toHaveLength(expectedTurns);
